...
function Login() {
    ...
    function registrarUsuario(e) {
        ...
        crearUsuario(usuario, password).then((user) => {
            login(usuario)
            dispararSweetBasico("Registro Correcto", "", "Sucess", "Confirmar")
        }).catch((error) => {
            if (error.code == "auth/invalid-credential") {
                dispararSweetBasico("Verifique los datos ingresados", "", "error", "Cerrar")
            } if (error.code == "auth/weak-password") {
                dispararSweetBasico("Contraseña debil", "La contraseña debe ser mayor a 6 caracteres", "error", "Cerrar")
            }
            //alert("Error")
        })
    }
    ...
    function iniciarSesionEmailPass(e) {
        ...
        loginEmailPass(usuario, password).then((user) => {
            login(usuario)
            dispararSweetBasico("Inicio Correcto", "", "success", "Confirmar")
        }).catch((error) => {
            if (error.code == "auth/invalid-credential") {
                dispararSweetBasico("Verifique los datos ingresados", "", "error", "Cerrar")
            }
    ...
    if (user || admin) {
        ...
        return (
            <div style={{ fontSize: "1.4vw", textAlign: "center", gap: "1vw", fontFamily: "calibri" }}>
                <div className='d-flex flex-column  justify-content-center  align-items-center min-vh-100'>
                    <form onSubmit={iniciarSesionEmailPass} className="p-4 border rounded shadow ">
                        ...
                        <button type="submit" className="btn btn-primary w-50">Aceptar</button>
