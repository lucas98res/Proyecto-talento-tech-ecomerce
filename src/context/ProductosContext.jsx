...
export function ProductosProvider({ children }) {
    ...
    const agregarProducto = (producto) => {
        return (
            new Promise(async (res, rej) => {
                try {
                    const respuesta = await fetch('https://683264bbc3f2222a8cb22f0d.mockapi.io/Pre1/Articulos', {
                        method: 'POST',
                        headers: {
    ...
    function editarProducto(producto) {
        return (
            new Promise(async (res, rej) => {
                try {
                    const respuesta = await fetch(`https://683264bbc3f2222a8cb22f0d.mockapi.io/Pre1/Articulos/${producto.id}`, {
                        method: 'PUT',
                        headers: {
                            'Content-Type': 'application/json',
                        },
    ...
    const eliminarProducto = (id) => {
        ...
        if (confirmar) {
            return (
                new Promise(async (res, rej) => {
                    try {
                        const respuesta = await fetch(`https://683264bbc3f2222a8cb22f0d.mockapi.io/Pre1/Articulos/${id}`, {
                            method: 'DELETE',
                        });
